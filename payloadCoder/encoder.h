...
private:
  ...
  /**
   * Adds a uint16_t value to the buffer at the specified index.
   *
   * @param idx_in The index in the buffer where the value should be added.
   * @param value The uint16_t value to be added to the buffer.
   * @return First free location at which new dat acan be stored in _buffer
   */
  unsigned char add_uint16(unsigned char idx_in, const uint16_t value);

  /**
   * Adds a uint32_t value to the buffer at the specified index.
   *
   * @param idx_in The index in the buffer where the value should be added.
   * @param value The uint32_t value to be added to the buffer.
   * @return First free location at which new dat acan be stored in _buffer
   */
  unsigned char add_uint32(unsigned char idx_in, uint32_t value);

  /**
   * Adds a boolean value to the payload buffer at the specified position.
   *
   ...
   * If the position is 0, the index is incremented so that
   * the next boolean value is added to the next byte in the buffer.
   *
