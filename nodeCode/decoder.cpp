 ...
 * and bit-level extraction for boolean flags.
 ...
 * Initializes all internal data members to zero or nullptr.
 ...
 * Currently, no dynamic memory is managed directly by this class,
 ...
 * `_buffer`. The extraction assumes a specific byte order (MSB first for multi-byte types)
 * and field layout:
 * - Bytes 0-3: ID (uint32_t)
 * - Byte 4: Version (uint8_t)
 * - Byte 5: Status bits
 *   - Bit 2: Door Status (1 = closed, 0 = open)
 *   - Bit 1: Catch Detect (1 = detected, 0 = not detected)
 *   - Bit 0: Trap Displacement (1 = displaced, 0 = not displaced)
...
void payloadDecoder::decodePayload()
{
    ...
    */
    ...
    // Ensure buffer is not null and size is adequate before decoding
    if (!_buffer || _bufferSize < 11) { // Minimum expected size for all fields
        // Handle error: print to debugSerial or set an error state
        #if defined(ARDUINO)
        if(debugSerial) debugSerial.println("Error: Payload buffer not set or too small.");
        #else
        std::cerr << "Error: Payload buffer not set or too small." << std::endl;
        #endif
        return;
    }
...
/**
 * @brief Prints the decoded payload fields and their values to the debug serial interface.
 * This function is useful for debugging and verifying that the payload has been
 * decoded as expected. Output format varies slightly for Arduino vs. other platforms.
 */
